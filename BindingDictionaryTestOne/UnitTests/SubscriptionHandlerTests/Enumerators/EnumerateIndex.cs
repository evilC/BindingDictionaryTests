...
namespace UnitTests.SubscriptionHandlerTests.Enumerators
{
    /// <summary>
    /// Given I have a SubscriptionHandler
    /// When I add or remove subscriptions
    /// Then the [BindingType] enumerator should return the correct number of subscribed BindingTypes
    /// </summary>

    [TestFixture]
    public class EnumerateIndex
    {
        ...
        public int CountIndexesTests(SubscriptionType subscriptionType, InputSubReq sr)
        {
            ...
            try
            {
                foreach (var index in _subHelper.SubHandler[sr.BindingDescriptor.Type])
                {
                    c++;
                }
            }
